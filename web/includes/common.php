...
class display_item
{
  ...
  function __set($member, $value)
  {
    switch ($member)
    {
      ...
      case "name":
        if (!is_string($value))
          trigger_error(var_export(debug_backtrace(), true), E_USER_ERROR);
        else
          $this->$member = $value;
        break;
      case "length":
...
{ 
  ...
  {
    debug_show(3, var_export($header_info, true));
    $head_count = 0;
    foreach ($header_info as $header)
    {
      $head_count++;
      if (!isset($header->type))
        echo "Unset header type (count: {$head_count}): " . var_export($header, true);
...
{
  //global $DEBUG, $link;
  if (isset($link)) { debug_show(2, var_export($link, true)); $link->close(); }
  echo "</div>\n</body>\n</html>";
